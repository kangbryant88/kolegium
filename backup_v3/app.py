...
def descargar_inscripcion_inicial(grado_id):
    ...
    # Definición de anchos de las 18 columnas (~335mm disponibles, sumaremos 332)
    w = [6, 38, 16, 14, 8, 8, 24, 8, 38, 17, 18, 50, 45, 7, 7, 7, 7, 14]
    ...
    headers = ["N°", "Apellidos y Nombres", "C.I. / C.E.", "F. Nac.", "Edad", "Sexo", "Procedencia", "Repet.", "Representante", "C.I. Rep.", "Teléfono", "Correo Representante", "Dirección Habitación", "Lit.", "Talla", "Peso", "Calz.", "Lateralidad"]
    ...
    for i, est in enumerate(estudiantes, 1):
        ...
        nombre_str = (est.nombre_completo[:28] + '..') if len(est.nombre_completo) > 28 else est.nombre_completo
        rep_str = (rep_nombre[:28] + '..') if len(rep_nombre) > 28 else rep_nombre
        email_str = (rep_email[:40] + '..') if len(rep_email) > 40 else rep_email
        dir_str = (rep_dir[:32] + '..') if len(rep_dir) > 32 else rep_dir
        proc = est.plantel_procedencia or est.procedencia or est.institucion_procedencia
        proc_str = (proc[:18] + '..') if proc and len(proc) > 18 else (proc or "Ninguna")
        
        pdf.cell(w[0], 6, str(i), border=1, align='C')
        pdf.cell(w[1], 6, nombre_str, border=1, align='L')
        ...
        pdf.cell(w[6], 6, proc_str, border=1, align='L')
        pdf.cell(w[7], 6, "Sí" if est.es_repetidor else "No", border=1, align='C')
        pdf.cell(w[8], 6, rep_str, border=1, align='L')
        pdf.cell(w[9], 6, str(rep_ci), border=1, align='C')
        pdf.cell(w[10], 6, rep_tlf, border=1, align='C')
        pdf.cell(w[11], 6, email_str, border=1, align='L')
        pdf.cell(w[12], 6, dir_str, border=1, align='L')
        ...
        pdf.cell(w[17], 6, str(est.lateralidad or '-'), border=1, align='C')
        pdf.ln()
